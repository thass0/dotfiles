...
# Display a summary of all tracked config files
config-ls(){
    (cd $HOME
     for i in $(config-git ls-files); do
	 echo -n "$(config-git -c color.status=always status $i -s | sed "s#$i##")"
	 echo -e "¬/$i¬\e[0;33m$(config-git -c color.ui=always log -1 --format="%s" -- $i)\e[0m"
     done
...
function git() {
  if [[ $@ == "ctx" ]]; then
    command git log -p --all --decorate --graph
  elif [[ $@ == "log" ]]; then
...
# ls should show hidden files by default.
function ls() {
    if  [[ $PWD == $HOME ]]; then
      command ls $@
  else
